...
│   ├── DELTA-FORMAT.md
│   └── deltas/<language>.md        # Additions + Overrides per language
└── patterns/                       # Regex pattern definitions for the static scanner
    ├── ai-security.yaml
    ├── api-auth.yaml
    ├── auth.yaml
    ├── compliance.yaml
...
Every skill in this catalog lives under `skills/_masters/`. There are no role
categories, no per-category index and no triage step that assigns roles: a
pipeline names a master, the master runs one agentic loop, and it fans out to
sub-agents itself when it needs to. That is the whole model.

Frontmatter each master declares:
...
| `activates_when`| Optional guard over `concept-vocabulary.yaml` keys                        |

`scripts/validate-skills.sh` enforces the required fields, the name/directory
match, the description cap and the principles templates. Run it before opening
a PR — CI runs the same script.

`principles/` is content, not a skill: `core.md` states intent with no mechanism
words, and each `deltas/<language>.md` carries the language-specific Additions
and Overrides. agent-smith composes them per detected language.
